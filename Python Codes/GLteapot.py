...
def draw_background(img):
        back_img = cv2.flip(img, 0)
        back_img = Image.fromarray(back_img)
        ix = back_img.size[0]
        iy = back_img.size[1]
        back_img = back_img.tobytes('raw', 'BGRX', 0, -1)
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, 3, ix, iy, 0, GL_RGBA, GL_UNSIGNED_BYTE, back_img)
        glBindTexture(GL_TEXTURE_2D, texture_background)
        glPushMatrix () 
...
def init_object_texture(image_filepath):
        ...
        back_obj = cv2.flip(tex, 0)
        back_obj = Image.fromarray(back_obj)
        ix = back_obj.size[0]
        iy = back_obj.size[1]
        back_obj = back_obj.tobytes('raw', 'BGRX', 0, -1)

        # create texture
        glBindTexture(GL_TEXTURE_2D, texture_object)
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST)
        glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST)
        glTexImage2D(GL_TEXTURE_2D, 0, 3, ix, iy, 0, GL_RGBA, GL_UNSIGNED_BYTE, back_obj)
        glBindTexture(GL_TEXTURE_2D, texture_object)
        return None
...
def overlay(img, ar_list, ar_id, texture_file):
        ...
        view_matrix = np.array([[rmtx[0][0],rmtx[0][1],rmtx[0][2], tvec[0][0][0]],
                                    [rmtx[1][0],rmtx[1][1],rmtx[1][2], tvec[0][0][1]/2], # div by 2 as my quads x is 6 and y is 3.
